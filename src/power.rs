...
impl<R: RegisterAccess> Rk3588PowerManager<R> {
    ...
    /// 控制电源域
    pub fn control_power_domain(&mut self, domain: PowerDomain, state: PowerState) -> PowerResult<()> {
    ...
    pub fn set_cpu_frequency(&mut self, domain: PowerDomain, freq: CpuFreq) -> PowerResult<()> {
        match domain {
            PowerDomain::CpuLittle => {
                info!("Setting CPU little cluster frequency to {} MHz, {} mV", 
                     freq.freq_mhz, freq.voltage_mv);
                self.cpu_little_freq = freq;
            }
            PowerDomain::CpuBig => {
                info!("Setting CPU big cluster frequency to {} MHz, {} mV", 
                     freq.freq_mhz, freq.voltage_mv);
    ...
    pub fn thermal_management(&mut self) -> PowerResult<()> {
        ...
        if temperature > 80.0 {
            warn!("Temperature high: {:.1}°C, implementing thermal throttling", temperature);
            ...
            // 2. 如果温度过高，关闭非必要组件
            if temperature > 85.0 {
                error!("Critical temperature: {:.1}°C, shutting down non-essential domains", temperature);
