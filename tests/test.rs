...
mod tests {
    ...
    fn test_dependency_manager_creation() {
        ...
        assert!(active.is_empty(), "New manager should have no active domains");
    }
    ...
    fn test_dependency_manager_mark_active() {
        ...
        let active = manager.get_active_domains();
        assert!(active.contains(&domain), "Domain should be marked as active");
    }
    ...
    fn test_dependency_manager_mark_inactive() {
        ...
        let active = manager.get_active_domains();
        assert!(!active.contains(&domain), "Domain should be marked as inactive");
    }
    ...
    fn test_qos_state_management() {
        ...
        assert!(!pm.has_qos_state(RK3588::GPU), "Should have no QoS state initially");
    ...
    fn test_parent_child_dependency_power_off_order() {
        ...
        let active = pm.get_active_domains();
        assert!(!active.contains(&RK3588::VCODEC), "Parent should be inactive");
    ...
    fn test_complex_vcodec_hierarchy() {
        ...
        let result = pm.power_domain_off_with_deps(RK3588::VCODEC);
        assert!(matches!(result, Err(PowerError::DependencyNotMet)), 
    ...
    fn test_get_active_domains_tracking() {
        ...
        assert_eq!(pm.get_active_domains().len(), 0, "Should start with 0 active domains");
        
        // Power on some domains
        pm.power_domain_on_with_deps(RK3588::VI).unwrap();
        assert_eq!(pm.get_active_domains().len(), 1, "Should have 1 active domain");
        
        pm.power_domain_on_with_deps(RK3588::ISP1).unwrap();
        assert_eq!(pm.get_active_domains().len(), 2, "Should have 2 active domains");
        ...
        assert_eq!(pm.get_active_domains().len(), 0, "Should have 0 active domains");
    ...
    fn test_qos_configured_domains() {
        ...
        assert!(!pm.has_qos_state(RK3588::GPU), "QoS state should not exist before power off");
