...
mod tests {
    ...
    fn test_cpu_frequency_control() {
        ...
        // 测试设置 CPU 小核频率
        let result = power_manager.set_cpu_frequency(
        ...
        // 验证频率设置
        let freq = power_manager.get_cpu_frequency(PowerDomain::CpuLittle).unwrap();
        ...
        let result = power_manager.set_cpu_frequency(
            PowerDomain::CpuBig, 
            cpu_freqs::FREQ_2208M
        );
        
        assert!(result.is_ok());
        
        let freq = power_manager.get_cpu_frequency(PowerDomain::CpuBig).unwrap();
